...
In this chapter we would like to compare a Linear Model, a Generalised Additive Model, a Regression Tree and a Neural Network with the help of crossvalidation. All these models allow to predict the logged prize of the real estates. We tried to make this as fair as possible. Yet, some of the models have additional parameters, that can have a huge influence on the result. We used for all models the same variables and tried to keep them untransformed when possible. 
...
# Add log price
house_models$log_price<-log(house_models$price)
...
For the crossvalidation we built a training and a test set. Then we built a model for each approach and calculated the squared residuals for the predicted values. We performed only one run, because our computional power is limited. So our result won't be robust. In practice we would need to perform this about 100 times and take the mean value for each model. In addition the best model parameters for each approach would have been evaluated in advance. 
...
# Linear Model
lm.fit<- lm(log_price ~ object_type_name + living_area +num_rooms, data=house_models.train)

predicted.lm <- predict(lm.fit,newdata = house_models.test) 

cor(predicted.lm, house_models.test$log_price)^2

# Generalised Additive Model
library(mgcv)
gam.fit<-gam(log_price ~ object_type_name + s(living_area) +num_rooms, data=house_models.train)

predicted.gam <- predict(gam.fit,newdata = house_models.test) 

cor(predicted.gam, house_models.test$log_price)^2

# Regression Tree
library(ISLR)
library(tree)
tree.fit<-tree(log_price ~ object_type_name + living_area +num_rooms, data=house_models.train, mindev=0.005)

predicted.tree <- predict(tree.fit,
newdata = house_models.test) 

cor(predicted.tree, house_models.test$log_price)^2

# Neural Network
library(neuralnet)

neural.fit<-neuralnet( log_price ~ object_type_name.num + living_area +num_rooms, data=house_models.train, hidden = c(3), linear.output = TRUE ,stepmax = 1e+8)

predicted.neural <- predict(neural.fit,
newdata = house_models.test) 

cor(predicted.neural, house_models.test$log_price)^2

```

So, after the first run we would chose the General Additive Model as the best model, but as stated before we would need to perform this approach multiple times to get a robust result.

\pagebreak

# Conclusion

We developed at least one working model for each chapter from the lessons. Furthermore we compared some Regression models with a cross validation. It doesn't make sense to compare all of the models, because they are sometimes only suitable for special cases like the count data in the Generalised Linear Models chapter. Overall we had to learn, that real data can be hard to model and the computional limitation with more sophisticated approaches like neural networks can be very frustrating. 