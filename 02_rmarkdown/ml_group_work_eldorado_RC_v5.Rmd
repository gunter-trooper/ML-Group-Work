...
First some basic libraries are loaded.In addition some RMarkdown options are defined and the sampling is set, so it is like in the past before R version 3.6. 
Finally the data is imported and the columns of interest are selected. 
...
So far, we have assumed all effects of continuous predictors to be linear. In this chapter we will leave linearity and look for non-linear relationships in the data. 
...
As an example let's compare the dependency of price from number of rooms. In order to achieve a better graphical overview, we will limit the number of rooms to be considered to 15 and delete all zeros, as it doesn't make sense that any real estate has zero rooms.
...
## Build Generalised Additive Models (GAMs)
...
Lets start with the binomial model. In this section we would like to evaluate for what kind of data this model is the better choice compared to the basic Linear model. 
...
Lets fit the data with a Possion model. By using this apporach the data is transformed with the exponential function to avoid negative values. Furthermore we assume a Possion distribution so the variance of the observations depends on the mean values and the simulated values are all integers. 
...
We will transform the data with the logit function. With this function the values are always between 0 and 1. We achieve this by fitting the data with a Binomial model. In addition we assume a Binomial distribution. 
...
As we see, there were 9131 object with the type "Others" and 13439 with the type "Wohnung". The model classifies 11992 out of 13439 "Others" and 2748 "Wohnung" out of 9131 correctly. This means that our Binary model classifies the object type "Others" way better than the object type "Wohnung". 
...
First we prepare the data by chosing the right data format. Furthermore the data ist reduced by limiting it to only entries with four rooms and the object types "Wohnung" and "Einfamilienhaus". The final dataset is splitted into a training and test dataset in the ratio 1:1. 
...
```{r}
nn2 <- neuralnet( object_type_name~ living_area +log_price+population_in_hectare,
                  hidden = c(3,3), data = house_data.network.train, 
                  linear.output = FALSE,stepmax = 1e+8)
summary(nn2)
...
As we see in the table, there were 491 "Einfamilienhaus" and 1052 "Wohnung" entries to be classified. Our model predicted 316 "Einfamilienhaus" correctly and only 85 "Wohnung" correctly. So this model is not really saitsfying. Lets try another neural network model with less hidden layers. 
...
```{r}
nn <- neuralnet( object_type_name~ living_area +log_price+population_in_hectare,
                 hidden = c(3), data = house_data.network.train, 
                 linear.output = FALSE,stepmax = 1e+8)
summary(nn)
...
As we see in the table, there were 491 "Einfamilienhaus" and 1052 "Wohnung" entries to be classified. Our model predicted 297 "Einfamilienhaus" correctly and only 96 "Wohnung" correctly. So this model is not really satisfying aswell. It classifies a little worse than the more complex one. 

Overall it seems, that used approach and data can't be used to distinguish properly between the two object types. The next step would be to add more variables to the moddeling process, add more layers or allows backpropagation. Otherwise an approach like Support Vector Machine can be tested as desribed earlier in this project report. 

\pagebreak

# Crossvalidation of the Regression models

In this chapter we would like to compare a Linear Model, a Generalised Additive Model, a Regression Tree and a Neural Network with the help of crossvalidation. All these models allow to predict the logged prize of the real estates. We tried to make this as fair as possible. Yet, some of the models have additional parameters, that can have a huge influence on the result.  

## Preparation

First we prepare the data again. 

```{r warning=FALSE}

# Save the data as a topic specific dataset
house_models<-house_data

house_models<-house_models%>%
  filter(num_rooms>0 & num_rooms<15)

# Add log price
house_models$price.log<-log(house_models$price)

# Room as factor
house_models$num_rooms.fac <- factor(house_models$num_rooms)
house_models$zipcode.fac <- factor(house_models$zipcode)

# object_type_name as numeric
unique(house_models$object_type_name)

getnum<-function(n) {
   if(n == "Wohnung") {
      1
    } else if(n == "Einfamilienhaus") {
      2
    } else if(n == "Mehrfamilienhaus") {
      3
    } else {
      4
    }
...
house_models$object_type_name.num <- sapply(house_models$object_type_name,getnum)

```

## Compare the models

For the crossvalidation we built a training and a test set. Then we used the implemented parameters from the previous chapters to built a model for each approach and calculated the squared residuals for the predicted values. For the neural network we didn't used optimized parameters, because we only performed a classifier neural network before. In addition we performed only one run, because our computional power is limited. So our result won't be robust. In practice we would need to perform this about 100 times and take the mean value for each model. In addition the best model parameters for each approach would have been evaluated in advance. 
...
train.YES <-sample(x=c(T,F), size = nrow(house_models),replace=T)

house_models.train <- house_models[train.YES, ]
house_models.test <- house_models[!train.YES, ]
```

```{r warning=FALSE}
# Linear Model
lm.fit.3 <- lm(price.log ~ object_type_name + build_year + living_area 
               + num_rooms +  number_of_apartments_in_hectare 
               + number_of_workplaces_in_hectare + population_in_hectare 
               + build_year:living_area + build_year:travel_time_private_transport 
               + build_year:number_of_buildings_in_hectare 
               + build_year:number_of_apartments_in_hectare 
               ...
               + number_of_apartments_in_hectare:population_in_hectare 
               + number_of_workplaces_in_hectare:population_in_hectare
               , data = house_models.train)

predicted.lm <- predict(lm.fit.3,newdata = house_models.test) 

lm.rsquared <- cor(predicted.lm, house_models.test$price.log)^2

```

R^2 Linear Model: `r lm.rsquared`  

```{r warning=FALSE}
# Generalised Additive Model
library(mgcv)
gam.price.1 <- gam(price.log ~ object_type_name + s(build_year) + s(living_area) + s(num_rooms) + s(number_of_apartments_in_hectare)
+ s(number_of_workplaces_in_hectare) + s(population_in_hectare),data = house_models.train)

predicted.gam <- predict(gam.price.1,newdata = house_models.test) 

gam.rsquared <- cor(predicted.gam, house_models.test$price.log)^2
```

R^2 Generalised Additive Model: `r gam.rsquared`  
...
predicted.tree <- predict(house_data_trees.model.boost,
newdata = house_models.test, n.trees=5000) 

predicted.tree.rsquared <- cor(predicted.tree, house_models.test$price.log)^2

```

R^2 Regression Tree (Boosting): `r predicted.tree.rsquared`  

```{r warning=FALSE}
# Neural Network
library(neuralnet)

neural.fit<-neuralnet( price.log ~ object_type_name.num + living_area +num_rooms, data=house_models.train, hidden = c(3), linear.output = TRUE ,stepmax = 1e+8)

predicted.neural <- predict(neural.fit,
newdata = house_models.test) 


neural.fit.rsquared <- cor(predicted.neural, house_models.test$price.log)^2

```

R^2 Neural Network: `r neural.fit.rsquared`  

So, after the first run we would chose the Regression Tree model as the best model, but as stated before we would need to perform this approach multiple times to get a robust result.

\pagebreak

# Conclusion

We developed at least one working model for each chapter from the course. Furthermore we compared some Regression models with a cross validation. It doesn't make sense to compare all of the models, because they are sometimes only suitable for special cases like the count data in the Generalised Linear Models chapter. Overall we had to learn, that real data can be hard to model and the computional limitation with more sophisticated approaches like neural networks can be very frustrating. 