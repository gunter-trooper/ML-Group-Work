...
```{r}
house_data <- read.csv("C:/Users/lucac/OneDrive/Dokumente/GitHub/ML-Group-Work/Machine-learning-databases/house_data.csv")
```
...
The scatterplot already give a first indication: Most of the points seem to be "sit at the bottom" of the scatterplot, here between 1 and 2 millions. 
...
```{r}
ggplot(house_data, aes(object_type_name, price)) + geom_boxplot()
```
Here we can clearly see, that "Einfamilienhäuser" seem to be the most valuable object. At this point in time we can not say, whether this difference is significant.Other than that there seem to be a couple outliers in the high price ranges.
...
Now we can check the correlation between all numberic input variables and the output variable price:
```{r}
cor(house_data_numeric)[,"price"]
```
...
library(ggplot2)
x %>%
  mutate(rowname = factor(rowname, levels = rowname[order(price)])) %>% # Order by correlation strength
